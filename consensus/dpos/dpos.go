...
var (
	...
	FrontierBlockReward  *big.Int = big.NewInt(3e+14) // Block reward in wei for successfully mining a block
...
func (d *Dpos) verifySeal(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	dposContext, err := dao.NewDposContextFromProto(d.db, parent.DposContext)
	if err != nil {
		return err
	}
...
func (d *Dpos) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction,
	uncles []*types.Header, receipts []*types.Receipt, dposContext *dao.DposContext) (*types.Block, error) {
...
func (d *Dpos) CheckValidator(lastBlock *types.Block, now int64) error {
	...
	dposContext, err := dao.NewDposContextFromProto(d.db, lastBlock.Header().DposContext)
	if err != nil {
		return err
	}
	epochContext := &EpochContext{DposContext: dposContext}
...
func updateMintCnt(parentBlockTime, currentBlockTime int64, validator common.Address, dposContext *dao.DposContext) {
